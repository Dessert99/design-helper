...
프로젝트의 실제 스타일 체계를 먼저 탐지한다. 기존 대상을 고칠 때는 그 체계의 값을 쓰고, 사용자에게 `프로젝트 유틸리티`, `프로젝트 CSS 변수`, `임의값 탐색안` 중 무엇으로 그렸는지 알린다.
...
Tailwind 클래스는 표본의 래퍼가 아닌 실제 `.target` 노드들에 적용해야 한다. 래퍼에만 붙이면 후보별 값이 컴포넌트에 도달하지 않을 수 있다. 워처 없이 클래스만 추가하면 빌드 결과에 그 클래스가 없을 수도 있다.

지침은 프로젝트 안에 비교용 config나 helper CSS를 만들지 않도록 한다. 기존 체계 안에서 볼지 새 방향을 찾을지 모르겠으면 먼저 묻는다. 다만 기존 디자인의 방향 재탐색은 [지침 간 범위 차이](limitations.md#방향재탐색)가 있다.
...
서버는 `127.0.0.1`에 바인딩한다. 기본 포트가 사용 중이면 다른 프로세스를 죽이지 않고 다음 포트를 찾는다. 시트의 HTML 변경을 HEAD 요청으로 감지해 새로고침하고, 새 section이 붙으면 그 비교로 스크롤한다. 범위·맥락 정보는 같은 탭의 `sessionStorage`에서 복원한다. 다른 탭에서 조작한 상태까지 맞춰 주지는 않는다.

시트를 수정하기 전에 서버가 살아 있는지 확인한다. 서버만 종료되고 작업 폴더가 남아 있으면 같은 폴더에서 다시 띄운다. 서버가 없는 상태로 파일만 바꿔 놓고 사용자 화면이 갱신되었다고 보고하지 않는다.

수정 시안을 제시할 때마다 비교 탭을 선택하고 브라우저 창을 앞으로 가져온다. 탭이나 브라우저가 닫혔다면 같은 세션의 시트 URL을 다시 연다. 서버만 종료되었으면 기존 작업 폴더에서 복구한다. 요청 없이 2분이 지나 폴더까지 삭제되었으면 코드와 대화 내용을 바탕으로 새 시안을 만든 뒤 연다. 기존 비교 탭이 있으면 그 탭을 다시 쓰고, 탭이 없음을 확인한 경우에만 새로 연다. 새 비교든 기존 구역을 고친 것이든 그 구역으로 스크롤해 보여 준다. 중간 파일 저장이나 대화만 하는 턴에는 포커스를 빼앗지 않는다.

자동 새로고침이 됐다고 화면이 앞으로 왔다고 보지 않는다. 탭 확인이나 활성화 도구가 없으면 중복 가능성이 있는 URL 열기로 우회하지 않고 한계와 현재 URL을 알린다. 운영체제 URL 열기는 비교 탭이 없음을 확인한 경우에만 사용한다. 화면 전환을 확인할 수 없으면 확인했다고 보고하지 않는다. 자세한 절차는 [수정 시안 제시](../../skills/design-helper/references/liveview.md#present-every-revision)를 따른다.
...
아무 말 없이 탭이나 세션이 끝난 경우에는 서버의 idle 감시가 작동한다. 구현은 탭의 존재 자체가 아니라 **마지막 요청 이후 경과 시간**을 본다. 요청이 약 2분 이상 없으면 서버를 닫고 등록된 보조 프로세스에 종료 신호를 보낸 뒤 그 세션의 임시 폴더를 통째로 지운다. 시안·메모·스크린샷도 삭제되므로 이후에는 새로 생성해야 한다. 프로젝트 코드와 상위 폴더는 삭제하지 않는다. 그래서 탭이 열려 있어 새로고침 요청이 계속 들어오면, 사용자가 자리를 비워도 꺼지지 않는다.

SIGTERM을 넘겨주지 않는 래퍼로 띄운 워처는 남을 수 있다. 서버 강제 종료나 충돌로 정리 코드가 실행되지 못한 경우, 파일 삭제까지 보장하지 않는다.
